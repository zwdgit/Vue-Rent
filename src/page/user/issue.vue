<template>
  <div>
    ...
    <div v-if="activeTab === 'issue'">
      ...
      <mu-card v-for="(item, index) in issue_data" :key="item.type">

        <mu-list-item class="Top-list-item" :title="nickName + ' | ' + '#' + item.info.type" :describeText="item.info.releaseDate">
    ...
    <div v-if="activeTab === 'dotpraise'">
      ...
      <mu-card v-for="item in dotpraise_data" :key="item.id">
        <mu-card-header :title="item.name + ' | ' + '#' + item.type" :subTitle="item.date">
          <mu-avatar src="http://localhost:8081/icon.png" slot="avatar"/>
        </mu-card-header>
        <mu-card-text>
          {{item.source}}
...
<script>
  export default {
    ...
    created() {
      this.nickName=JSON.parse(sessionStorage.getItem("User_Data")).nickName;
      this.getIssue()
    ...
    methods: {
      ...
      getIssue() {
        ...
        param.append("id",id);
        let url='http://localhost:8081/api/user/' + this.activeTab;
        axios.get(url,{
          params:{
            id:id
          }
