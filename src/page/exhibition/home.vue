<template>
  <div>
    ...
    <slider :pagination-visible="true" :slides="swiper_data" :auto="0">
      <div style="position: relative" v-for="(slide,index) in swiper_data" :key="index">
        <a>
          ...
          <span class="title-mask" v-if="slide.title">
            <p>by {{slide.name}}</p>
            <p>{{slide.title}}</p>
            <mu-flat-button to="/exhibition/atlas" class="more-btn" labelPosition="before" label="更多" icon="more_vert"
                            primary/>
          </span>
        </a>
      </div>
    ...
    <div class="topic-class" @click="To_Potic">
      <div class="topic-class-background">
        ...
        <span class="topic-class-content">
                ...
                <p style="font-size: 1.2em"># 聆听工大的专属语言 #</p>
                <p style="color: #C9C9C9">嘿，你听过吗？</p>
            </span>
